...
python scripts/format_bibtex.py references.bib --sort year --descending --output sorted.bib
...
python scripts/validate_citations.py references.bib --auto-fix --output validated.bib

# Generate detailed validation report
python scripts/validate_citations.py references.bib --report validation.json --verbose
```

**Validation Checks** (see `references/citation_validation.md`):
1. DOI Verification via doi.org and CrossRef
2. Required fields presence
3. Data consistency (year, pages, volume)
4. Duplicate detection
5. Format compliance (BibTeX syntax)
...
pip install scholarly  # For Google Scholar
pip install crossref-commons pylatexenc  # Optional
```

## Summary

The citation-management skill provides:
1. Comprehensive search via Google Scholar and PubMed
2. Automated metadata extraction (DOI, PMID, arXiv, URL)
3. Citation validation with DOI verification
4. BibTeX formatting and cleaning tools
5. Integration with literature-review and scientific writing workflows
